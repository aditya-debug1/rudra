...
export const ClientSchema = ClientBaseSchema
  .superRefine(({ visitData }, ctx) => {
    const requiresOtherRef =
      visitData.reference === "reference" || visitData.reference === "other";

    if (requiresOtherRef && !visitData.otherRefs?.trim()) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["visitData", "otherRefs"],
